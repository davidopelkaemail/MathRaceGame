...
const Track = (function() {
  ...
  function createCar(colorHex) {
    ...
    // Headlights (Glowing Meshes)
    const headlightMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
    const hlLeft = new THREE.Mesh(new THREE.BoxGeometry(wVal * 0.15, 20 * scale, 10 * scale), headlightMat);
    hlLeft.position.set(-wVal*0.1, wheelRad + bodyHeight/2, lVal * 0.85);
    group.add(hlLeft);

    const hlRight = new THREE.Mesh(new THREE.BoxGeometry(wVal * 0.15, 20 * scale, 10 * scale), headlightMat);
    hlRight.position.set(wVal*0.1, wheelRad + bodyHeight/2, lVal * 0.85);
    group.add(hlRight);

    // Dynamic Headlight Casting (PointLight)
    const carLight = new THREE.PointLight(0xffffff, 2, 4000);
    carLight.position.set(0, wheelRad + bodyHeight, lVal);
    group.add(carLight);

    // Place Indicator Sprite
    // Set depthTest to true so they occlude properly behind cars in front
    ...
    // Raise higher above the car (increased by 50% of plate height)
    const spriteY = wheelRad + bodyHeight * 14.8;
    ...
    // Cast shadow from group root if needed
  ...
  function updateCars(data) {
    ...
    if (carMeshes.length !== data.length) {
      carMeshes.forEach(m => scene.remove(m));
      carMeshes = data.map(p => { const m = createCar(p.color.hex); scene.add(m); return m; });
    }
    data.forEach((p, i) => {
      const m = carMeshes[i];
