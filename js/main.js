...
const App = (function() {
      ...
      <div class="player-config">
        ...
        <div style="margin: 15px 0">
          ...
          <div style="display:flex; flex-direction:column; gap:10px">
            ${ops.map(o => `
              <div style="background:rgba(255, 255, 255, 0.03); padding:10px 15px; border-radius:12px; display:flex; justify-content:space-between; align-items:center; border: 1px solid rgba(255, 255, 255, 0.05)">
                <label style="display:flex; align-items:center; gap:10px; cursor:pointer; font-weight:600; font-size:15px">
                  <input type="checkbox" ${config.operations.includes(o) ? 'checked' : ''} data-player="${index}" data-op="${o}" class="op-checkbox" style="width:16px; height:16px">
                  ${o.charAt(0).toUpperCase() + o.slice(1)}
                </label>
                <select data-player="${index}" data-op="${o}" class="diff-select-inline">
                  ${diffs.map(d => {
                    const label = d === 'easy' ? 'Easy (up to 10)' : d === 'medium' ? 'Medium (up to 100)' : 'Hard (up to 1000)';
                    return `<option value="${d}" ${config.difficulties[o] === d ? 'selected' : ''}>${label}</option>`;
                  }).join('')}
                </select>
              </div>
            `).join('')}
  ...
  function setupConfigListeners() {
    ...
    elements.playerConfigsContainer.querySelectorAll('.diff-select-inline').forEach(s => s.addEventListener('change', e => {
